...
firebaseConfig = {
'Firebasete db oluşturduğunuz zaman verilen config verisi'
}

firebase = pyrebase.initialize_app(firebaseConfig)
auth = firebase.auth()
auth.sign_in_with_email_and_password("mailAdresi","Parola")
...
def dronBaglan():
    while True:
        try:
            baglantiAdresi = input("Baglanti Adresi (SITL icin genellikle udp:127.0.0.1:14551) : ")
            drone = connect(baglantiAdresi,wait_ready=True)
            print("Baglanti Saglandi...")
            return drone
        except:
            print("Baglantida bir hata olustu, tekrar deneyin...")
            return None
